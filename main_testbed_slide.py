...
import timm
import yaml
from src.pruning.coreset import CoresetSelection
import openslide
...
def main(args):
    ...
    model = timm.create_model(
        args.feature_extraction_model, pretrained=True
    ...
    for wsi_path in wsi_paths:
        ...
        slide_patch_dataset = SlidePatchesDataset(
            patch_dir=os.path.join(args.patch_path, slide_basename), transform=transform
        )

        dataloader = DataLoader(
            slide_patch_dataset, batch_size=args.batch_size, shuffle=True
        )
        ...
        slide_features = torch.cat(
            _slide_features, dim=0
        )  # Concatenate all features for the slide on GPU
        ...
        scores = torch.norm(slide_features, p=2, dim=1)
        data_score = {"score_key": scores}
        selected_indices, selected_data = CoresetSelection.score_monotonic_selection(
            data_score, slide_features
        )
        print(f"Shape of selected data: {selected_data.shape}")
        print(
            f"Indices after score_monotonic_selection: {selected_indices.tolist()[:5]}"
        )
        selected_indices_stratified, selected_data_stratified = (
            CoresetSelection.stratified_sampling(data_score, slide_features)
        )
        print(f"Shape of stratified selected data: {selected_data_stratified.shape}")
        print(
